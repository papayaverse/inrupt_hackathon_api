...
app.get("/data/:company1/share/:company2", async (req, res, next) => {
    const session = await getSessionFromStorage(req.session.sessionId);
    ...
    const buyerPod = await getPodUrlAll(buyerWebId);
    const buyerWalletAddress = buyerPod[0] + "testFolder/papayaWallet/wallet/avalanche/walletAddress.txt";
    const buyerWalletAddressBlob = await getFile(buyerWalletAddress, { fetch: session.fetch });
    const buyerWallet = await buyerWalletAddressBlob.text();
    if (session.info.isLoggedIn) {
        const podUrl = await getPodUrlAll(session.info.webId);
        const walletFolderUrl = podUrl[0] + "testFolder/papayaWallet/wallet/avalanche/";
        const walletAddressFileUrl = walletFolderUrl + "walletAddress.txt";
        ...
        if((companyName == buyerName) || (thirdPartyPref == "true")){ 
            tokenData = {
                "name": "Green papaya NFT for sharing data of " + username + " generated by interacting with " + companyName + " with " + buyerName,
                ...
                "greenpapaya_data_buyer": buyerWebId,
            ...
            const sdk = new SDK(auth);
            let greenPapayaAddress = "0xbC87DB14562E7468885DcBe4EF1113419166677e";
            ...
            console.log(nfts);
            tokenid = nfts.assets[0].tokenId;

            //sellNftAndChangeAccess;
            let dummy = await makeAgentReadAndAppend(dataFolderUrl, buyerWebId, session);
            const txTransfer = await greenPapayaContract.transfer({
                from: walletAddress,
                to: buyerWallet,
                tokenId: tokenid// Fill in with actual token id
             });
             receipt = await txTransfer.wait();
             console.log(receipt);
            return res.send("<p> Data of " + username + " generated by interacting with " + companyName + " is now shared with " + buyerName + ".</p>");
        }
    }
    else {
        return res.send("<p>Not logged in.</p>")
