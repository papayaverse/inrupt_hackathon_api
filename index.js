...
app.get("/data/:company1/showOwners", async (req, res, next) => {
    ...
    if (session.info.isLoggedIn) {
        ...
        console.log(owners);
        let owners2 = getSolidDataset(dataFolderUrl + "/tokens/owners", { fetch: session.fetch });
        owners2 = await getContainedResourceUrlAll(owners2);
...
app.get("/data/:username/accessNft/:company2", async (req, res, next) => {
    ...
    if (session.info.isLoggedIn) {
        ...
        const mintTx = await GreenPapayaContract.mint({
            ...
            cost: "0.0025",
            metadata: {
                greenpapaya_data_buyer: session.info.webId,
            }
        });
        const minted = await mintTx.wait();
        console.log('Mint Tx: ', minted);
        let companyname = session.info.webId.split("com/fake")[1];
        saveTextFile(userPod[0] + "testFolder/papayaData/" + company2Name + "/tokens/owners/" + companyname + ".txt", "True", session);
